...
from .models import PackageRequest, SessionMetadata, SessionPackage
...
class SessionPackager:
    ...
    def load_package(self, session_id: str) -> Optional[SessionPackage]:
        ...
        try:
            results = self.storage_db.execute_query(
                "SELECT package_data FROM packages WHERE session_id = ?", (session_id,)
            )
    ...
    def delete_package(self, session_id: str) -> bool:
        ...
        try:
            self.storage_db.execute_update(
                "DELETE FROM packages WHERE session_id = ?", (session_id,)
            )
    ...
    def create_session_package(
        self, request: PackageRequest, save_to_storage: bool = True
    ) -> SessionPackage:
        ...
        if request.include_sop:
            sop_documents.append(
                self.sop_generator.generate_sop(request.session_metadata)
            )

        # Generate IRB documents
        irb_documents = []
        if request.include_irb:
            irb_documents = self.irb_generator.generate_irb_package(
        ...
        if request.include_bids:
            bids_events = self.bids_generator._generate_events_template(
                request.session_metadata, request.custom_events
            )
    ...
    def export_package(
        self, package: SessionPackage, output_dir: Path, formats: List[str] = None
    ) -> Dict[str, Path]:
    ...
    def _export_pdf(self, package: SessionPackage, output_dir: Path) -> Path:
        ...
        if package.sop_documents:
            ...
            for sop in package.sop_documents:
                ...
                story.append(
                    Paragraph(f"<b>Purpose:</b> {sop.purpose}", styles["Normal"])
                )
                ...
                story.append(
                    Paragraph("<b>Safety Considerations:</b>", styles["Heading3"])
                )
        ...
        if package.irb_documents:
            ...
            for doc in package.irb_documents:
                story.append(
                    Paragraph(
                        doc.document_type.replace("_", " ").title(), styles["Heading2"]
                    )
                )
    ...
    def _export_docx(self, package: SessionPackage, output_dir: Path) -> Path:
        ...
        doc.add_paragraph(
            f"Principal Investigator: {package.session_metadata.principal_investigator}"
        )
        doc.add_paragraph(f"Modality: {package.session_metadata.modality.value}")
        doc.add_paragraph(
            f"Session Type: {package.session_metadata.session_type.value}"
        )
        doc.add_paragraph(
            f"Population: {package.session_metadata.participant_population.value}"
        )
        doc.add_paragraph(
            f"Duration: {package.session_metadata.duration_minutes} minutes"
        )
        doc.add_paragraph(
            f"Expected Participants: {package.session_metadata.expected_participants}"
        )
    ...
    def _export_bids(self, package: SessionPackage, output_dir: Path) -> Path:
        ...
        self.bids_generator.export_bids_package(
            package.session_metadata, bids_dir, package.bids_events
        )
        return bids_dir
    ...
    def _export_zip(self, package: SessionPackage, output_dir: Path) -> Path:
        ...
        zip_path = (
            output_dir / f"{package.session_metadata.session_id}_complete_package.zip"
        )
        ...
        for sop in package.sop_documents:
            ...
            if not sop.procedure_steps:
                validation_results["warnings"].append(
                    "SOP document missing procedure steps"
                )
